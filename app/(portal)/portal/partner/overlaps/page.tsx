...
export default function ServiceOverlapsPage() {
  ...
  useEffect(() => {
    if (useMockData) {
      setItems(mockServiceOverlaps as any[]);
      setLoading(false);
    } else {
      loadData();
    }
  ...
  const loadData = async () => {
    ...
    try {
      const q = query(collection(db, "serviceOverlaps"));
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-muted-foreground">Manage your service overlaps</p>
          </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Service Overlaps</CardTitle>
          <CardDescription>View and manage your service overlaps</CardDescription>
        </CardHeader>
        <CardContent>
          {items.length === 0 ? (
