...
function DialogContent({
  ...
  const hasDescription = React.Children.toArray(children).some((child) => {
    if (React.isValidElement(child)) {
      // Check for DialogDescription or DialogHeader that might contain one
      const slot = (child.props as { "data-slot"?: string })?.["data-slot"];
      if (slot === "dialog-description") return true;
      // Check nested children in DialogHeader
      if (slot === "dialog-header" && React.isValidElement(child)) {
        ...
        return React.Children.toArray(headerChildren).some(
          (headerChild) =>
            ...
            (headerChild.props as { "data-slot"?: string })?.["data-slot"] === "dialog-description"
        );
      }
    }
    return false;
